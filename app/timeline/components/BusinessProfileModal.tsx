...
interface BusinessProfileModalProps {
    onClose: () => void;
    onSubmit: (formData: Partial<Profile>) => void;
    isGenerating: boolean;
    initialData: Partial<Profile>;
}

export default function BusinessProfileModal({ onClose, onSubmit, isGenerating, initialData }: BusinessProfileModalProps) {
  ...
  const handleGoalToggle = (goal: string) => {
    setFormData(prev => {
        const currentGoals = (prev.primaryGoals as string[]) || [];
        return {
            ...prev,
            primaryGoals: currentGoals.includes(goal)
  ...
  const canProceed = (): boolean => {
    switch (currentStep) {
      ...
      case 2:
        return !!(formData.aiMaturityLevel && (formData.primaryGoals as any[])?.length > 0);
      case 3:
  ...
  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modalContainer} onClick={(e: MouseEvent) => e.stopPropagation()}>
            ...
            <div className={styles.formStep}>
              ...
              <div className={styles.formGroup}>
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => handleChange('industry', e.target.value)}
                  required
                >
                  ...
                  {companySizes.map(size => (
                    <label key={size.value} className={styles.radioLabel}>
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('companySize', e.target.value)}
                      />
            ...
            <div className={styles.formStep}>
                ...
                <div className={styles.goalsGrid}>
                  {primaryGoals.map(goal => (
                    <button
                      key={goal}
                      type="button"
                      className={`${styles.goalOption} ${(formData.primaryGoals as string[])?.includes(goal) ? styles.selected : ''}`}
              ...
              <div className={styles.formGroup}>
                ...
                <textarea
                  ...
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) => handleChange('currentChallenges', e.target.value)}
