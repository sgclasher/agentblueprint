...
/**
 * Extracts key strategic and financial data from the structured profile object.
 * This is a much more robust method than parsing markdown.
 * @param {object} profileData - The structured profile data object.
 * @returns {string} A summarized string of the most important data points.
 */
const extractKeyDataFromObject = (profileData) => {
  const keyData = [];
  ...
  if (expectedOutcome?.strategicInitiatives?.length > 0) {
    const goals = expectedOutcome.strategicInitiatives.map(i => `- ${i.initiative}`).join('\\n');
    keyData.push(`**Key Strategic Goals:**\\n${goals}`);
  }

  if (problems?.businessProblems?.length > 0) {
    const problemList = problems.businessProblems.map(p => `- ${p}`).join('\\n');
  ...
  if (currentArchitecture) {
    ...
    if (currentArchitecture.coreSystems?.length > 0) {
      techContext.push(...currentArchitecture.coreSystems.map(s => `- Core System: ${s}`));
    }
 ...
 */
export const buildTimelineUserPrompt = (profileData, scenarioType) => {
  ...
  // Generate the full markdown for context
  const profileMarkdown = markdownService.generateMarkdown(profileData);
  // Generate the key data summary from the object for focus
